...
interface BoardStoreState {
  ...
  updateBoard: (id: string, data: Partial<BoardInput>) => Promise<void>;
  removeBoard: (id: string) => Promise<void>;
}

const defaultRepository =
  process.env.NODE_ENV === "test"
    ? new MockBoardRepository()
    : new SQLiteBoardRepository();

export const createBoardStore = (
  repository: IBoardRepository = defaultRepository,
) => {
  return create<BoardStoreState>((set, get) => ({
    ...
    async updateBoard(id: string, data: Partial<BoardInput>) {
      set({ isLoading: true, error: null });
      try {
        await repository.updateBoard(id, data);
        set((state) => ({
          boards: state.boards.map((board) =>
            board.id === id
              ? { ...board, ...data, updatedAt: Date.now() }
              : board,
          ),
          isLoading: false,
        }));
