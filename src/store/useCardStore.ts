...
const defaultRepository = process.env.NODE_ENV === 'test' ? new MockCardRepository() : new DrizzleCardRepository()

export const createCardStore = (repository: ICardRepository = defaultRepository) => {
  return create<CardStoreState>((set, get) => ({
    ...
    async updateCard(
      id: string,
      data: Partial<Pick<Card, 'title' | 'description' | 'orderIndex' | 'listId' | 'status'>>,
    ) {
      set({ isLoading: true, error: null })
      ...
      try {
        ...
        set((state) => ({
          cards: state.cards.map((card) => (card.id === id ? { ...card, ...data, updatedAt: Date.now() } : card)),
          isLoading: false,
        }))
      } catch (error) {
    ...
    async moveCard(cardId: string, toListId: string, toIndex: number) {
      ...
      set((s) => ({
        cards: s.cards.map((c) => {
          ...
          if (update) return { ...c, listId: update.listId, orderIndex: update.orderIndex, updatedAt: Date.now() }
          return c
        }),
      }))
      ...
      try {
        for (const update of updates) {
          await repository.updateCard(update.id, { listId: update.listId, orderIndex: update.orderIndex })
        }
      } catch (error) {
