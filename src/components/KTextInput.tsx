...
export const KTextInput = forwardRef<TextInput, kTextInputProps>(
  ({ label, error, leftIcon, rightIcon, containerClassName = '', inputClassName = '', ...props }, ref) => {
    return (
      <View>
        {label && <Text className='mb-1 text-sm font-medium text-gray-700'>{label}</Text>}

        <View
          className={`flex-row items-center my-2 border-2 mb-6 rounded-xl border-neutral-200 dark:border-neutralDark-600 ${containerClassName}`}
        >
